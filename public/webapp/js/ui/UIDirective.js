...
// 用例：<pica-pagination ng-change="pageChangeCallback()" ng-model="currentPage" items-per-page="pageSize" total-items="totalItems" page-list="pageList"></pica-pagination>
app.directive('picaPagination', function() {
	return {
        restrict : "E",
		replace: true,
		require: '?ngModel',  // 指令中使用的其他指令，而不是指令模板中使用的指令。
		scope: {
		...
		templateUrl: 'template/template-pica-pagination.html',
		compile: function(element, attrs) {
        	element.addClass(attrs.wrapClass || 'text-right')
        	return function(scope, element, attrs, ctrl) {
        		/**
				 * 加此方法的原因：当在页面中改变页码或pageSize后只会更新当前指令scope中的model值。
				 * 在此方法中来手动更新父scope中的model.
				 * */
                scope.paperChangeCallback = function() {
                    ctrl.$setViewValue(scope.currentPage);
				}
				scope.sizeChangeCallback = function() {
                	scope.$parent[attrs.itemsPerPage] = scope.pageSize
